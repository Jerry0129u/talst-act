    ...
    const CHAIN = [
        ...
        { name: 'Нягтлан',      email: 'unumunkh@talstgroup.mn' },
        { name: 'Баярмаа',      email: 'bayarmaa@talstgroup.mn' },  // ← эцсийн мэдэгдэл
    ];
    ...
    const ROLES = {
        ...
        'unumunkh@talstgroup.mn':    'Ерөнхий Нягтлан',
        'bayarmaa@talstgroup.mn':    'Ерөнхий Нягтлан',   // ← НЭМЭХ
    ...
    // Persistence — нэвтрэлтийг localStorage/indexedDB-д хадгална
    (async () => {
        try {
    ...
    window.signInGoogle = async () => {
        try {
            ...
            // Desktop: popup эхлээд, хаагдвал redirect
            try {
                await signInWithPopup(auth, provider);
            } catch (popupErr) {
                console.warn('Popup амжилтгүй, redirect руу шилжиж байна:', popupErr?.code);
    
                if (popupErr?.code === 'auth/popup-blocked') {
                    toast('Popup хаагдсан байна. Redirect горимд шилжиж байна...', 'ok');
            ...
            if (err?.code === 'auth/unauthorized-domain') {
                toast('Энэ домэйн зөвшөөрөгдөөгүй.', 'err');
            } else {
    ...
    window.quickEbrPaid = (docId) => chooseEbrPaid(docId);
    window.quickEbrUnpaid = (docId) => chooseEbrUnpaid(docId);
    ...
    onAuthStateChanged(auth, u => {
        if (u) {
            ...
            const SHORT_NAMES = {
                ...
                'bayarmaa@talstgroup.mn':    'Баярмаа',            // ← НЭМЭХ
                'ulziisaikhan@talstgroup.mn':'CEO',
                'narankhuu@talstgroup.mn':   'CFO',
            };
    ...
    async function submitContract() {
        if (role !== 'Төслийн Координатор') { toast('Зөвхөн Төслийн Координатор гэрээ бүртгэх эрхтэй', 'err'); return; }
    ...
    function contractStats(c) {
        ...
        const isComplete = c.status === 'completed' || balance <= 0 || pctUsed >= 100;
        let barColor = '#00d9a3';
        if (isComplete) barColor = '#059669';
        else if (pctUsed >= 80) barColor = '#ffb547';
        return { total, used, balance, pctUsed, isComplete, barColor };
    }
    
    function contractCardHtml(c) {
        const { total, used, balance, pctUsed, isComplete, barColor } = contractStats(c);
        ...
        return `<div class="contract-card ${isComplete ? 'cc-done' : ''}" onclick="openContract('${c.id}')" style="cursor:pointer">
            <div class="cc-top">
                <div>
                    <div class="cc-no">${esc(c.contractNo)}${isComplete ? ' <span class="cc-done-badge">✓ Дууссан</span>' : ''}</div>
                    <div class="cc-company">${esc(c.company)}</div>
                </div>
                <div class="cc-balance ${isComplete ? 'cc-complete' : balance <= 0 ? 'cc-empty' : ''}">
                    <div class="cc-balance-label">${isComplete ? 'Төлөв' : 'Үлдэгдэл'}</div>
                    <div class="cc-balance-val">${isComplete ? '100%' : '₮' + fmtN(balance)}</div>
                </div>
            </div>
    ...
    function rContracts() {
        ...
        if (formCard) formCard.style.display = (role === 'Төслийн Координатор') ? 'block' : 'none';
        ...
        const active = contracts.filter(c => !contractStats(c).isComplete);
        const done = contracts.filter(c => contractStats(c).isComplete);
    
        function contractSectionHtml(label, items) {
            if (!items.length) return '';
            return `<div class="cc-section">
                <div class="cc-section-label">${label}</div>
                <div class="cc-section-grid">${items.map(contractCardHtml).join('')}</div>
            </div>`;
        }
    
        el.innerHTML =
            contractSectionHtml(`Идэвхтэй гэрээнүүд (${active.length})`, active) +
            contractSectionHtml(`✓ 100% дууссан гэрээнүүд (${done.length})`, done);
    }
        ...
        const html = `
            <div class="cdt-wrap">
              <div style="display:flex;justify-content:space-between;align-items:center;gap:10px">
                 ...
                 ${role === 'Төслийн Координатор' ? `<button class="cdt-back" onclick="editContract('${c.id}')" style="border-color:var(--brand,#635bff);color:var(--brand,#635bff)">✎ Засах</button>` : ''}
              </div>
                ...
                <div class="cdt-summary">
                    <div class="cdt-sum-head">
                        ...
                        <span class="cdt-sum-badge ${isComplete ? 'cdt-badge-green' : balance <= 0 ? 'cdt-badge-red' : 'cdt-badge-green'}">${isComplete ? '✓ 100% дууссан' : pctUsed + '% ашигласан'}</span>
                    </div>
                    ${isComplete ? `<div class="cdt-done-banner">✓ Энэ гэрээний дүн бүрэн төлөгдөж дууссан</div>` : ''}
                    <div class="cdt-sum-rows">
                        ...
                        <div class="cdt-sum-row cdt-sum-balance"><span>${isComplete ? 'Төлөв' : 'Үлдэгдэл'}</span><strong style="color:${isComplete ? '#059669' : balance <= 0 ? '#dc2626' : '#059669'}">${isComplete ? '✓ Дууссан' : '₮' + fmtN(balance)}</strong></div>
                    </div>
    ...
    // ════════════════════════════════════════════════════════════
    // ★ ГЭРЭЭ ЗАСАХ — зөвхөн Координатор
    // ════════════════════════════════════════════════════════════
    function editContract(contractId) {
        if (role !== 'Төслийн Координатор') { toast('Зөвхөн Төслийн Координатор гэрээ засах эрхтэй', 'err'); return; }
        const c = contracts.find(x => x.id === contractId);
        if (!c) { toast('Гэрээ олдсонгүй', 'err'); return; }

        const inp = 'width:100%;padding:11px 13px;border:1px solid var(--border-subtle,#e2e8f0);border-radius:10px;font-size:14px;margin-bottom:6px;box-sizing:border-box;background:var(--surface,#fff);color:var(--text-primary,#0f172a)';
        const lbl = 'display:block;font-size:12px;font-weight:600;color:var(--text-secondary,#64748b);margin:12px 0 4px';

        const html = `
        <div class="cdt-wrap">
            <button class="cdt-back" onclick="openContract('${c.id}')">← Буцах</button>
            <div class="cdt-summary">
                <div class="cdt-sum-head">
                    <div>
                        <div class="cdt-sum-no">Гэрээ засах</div>
                        <div class="cdt-sum-company">${esc(c.contractNo)}</div>
                    </div>
                </div>
                <div style="margin-top:14px">
                    <label style="${lbl}">Компани</label>
                    <input id="ceCompany" style="${inp}" value="${esc(c.company || '')}">

                    <label style="${lbl}">Гэрээний дугаар</label>
                    <input id="ceNo" style="${inp}" value="${esc(c.contractNo || '')}">

                    <label style="${lbl}">Нийт дүн (₮)</label>
                    <input id="ceAmount" type="number" style="${inp}" value="${esc(c.totalAmount || '')}" oninput="ceFmt()">
                    <div id="ceAmtFmt" style="font-size:13px;color:#059669;font-weight:600;min-height:18px"></div>

                    <label style="${lbl}">Ашигласан дүн (₮)</label>
                    <input id="ceUsed" type="number" style="${inp}" value="${esc(c.usedAmount || '0')}" oninput="ceFmt()">
                    <div id="ceUsedFmt" style="font-size:13px;color:#d97706;font-weight:600;min-height:18px"></div>
                    <div style="font-size:12px;color:#b45309;background:rgba(245,158,11,0.1);padding:8px 10px;border-radius:8px;margin:6px 0 14px">
                        ⚠ "Ашигласан дүн" нь акт батлагдах бүрт автоматаар нэмэгддэг. Зөвхөн засвар шаардлагатай үед л гараар өөрчилнө үү.
                    </div>

                    <div class="abtns" style="margin:0">
                        <button class="bok" id="ceSaveBtn" onclick="saveContractEdit('${c.id}')">💾 Хадгалах</button>
                        <button class="bno" onclick="openContract('${c.id}')">Цуцлах</button>
                    </div>
                </div>
            </div>
        </div>`;
        ...
        if (secLbl) secLbl.style.display = 'none';
        if (listEl) {
            listEl.classList.add('cdt-detail-mode');
            listEl.innerHTML = html;
        }
        ceFmt();
    }
    ...
    function ceFmt() {
        const a = e('ceAmount'), u = e('ceUsed');
        if (a) { const v = a.value; e('ceAmtFmt').textContent = v && !isNaN(v) ? '₮ ' + parseInt(v).toLocaleString() : ''; }
        if (u) { const v = u.value; e('ceUsedFmt').textContent = v && !isNaN(v) ? '₮ ' + parseInt(v).toLocaleString() : ''; }
    }
    ...
    async function saveContractEdit(contractId) {
        ...
        const company = e('ceCompany').value.trim();
        const no = e('ceNo').value.trim();
        const amount = e('ceAmount').value.trim();
        const used = e('ceUsed').value.trim();

        if (!company || !no || !amount) { toast('Компани, дугаар, нийт дүнг бөглөнө үү', 'err'); return; }
        if (isNaN(amount) || parseInt(amount) <= 0) { toast('Нийт дүн зөв оруулна уу', 'err'); return; }
        if (used !== '' && (isNaN(used) || parseInt(used) < 0)) { toast('Ашигласан дүн зөв оруулна уу', 'err'); return; }

        // Давхардсан дугаар шалгах (өөрийгөө эс тооцон)
        if (contracts.some(x => x.id !== contractId &&
            (x.contractNo || '').toLowerCase().trim() === no.toLowerCase().trim())) {
            toast('Энэ дугаар өөр гэрээнд бүртгэлтэй байна', 'err');
            return;
        }

        const btn = e('ceSaveBtn');
        if (btn) { btn.disabled = true; btn.textContent = 'Хадгалж байна...'; }

        try {
            const newTotal = parseInt(amount);
            const newUsed = used === '' ? parseInt(c.usedAmount || 0) : parseInt(used);

            const upd = {
                company, contractNo: no,
                totalAmount: String(newTotal),
                usedAmount: String(newUsed),
                editedBy: cu.email,
                editedByName: cu.displayName || cu.email,
                editedAt: serverTimestamp()
            };

            // Дүн дуусгасан эсэхийг дахин тооцох
            if (newTotal > 0 && newUsed >= newTotal) {
                upd.status = 'completed';
                if (!c.completedAt) upd.completedAt = serverTimestamp();
            } else {
                upd.status = null;        // дахин идэвхтэй болгоно
                upd.completedAt = null;
            }

            await updateDoc(doc(db, 'contracts', contractId), upd);
            toast('✅ Гэрээ шинэчлэгдлээ!');
            openContract(contractId);
        } catch (err) {
            console.error('Contract edit error:', err);
            toast('Алдаа: ' + err.message, 'err');
            if (btn) { btn.disabled = false; btn.textContent = '💾 Хадгалах'; }
        }
    }
    ...
    function renderContractBalances() {
        ...
        el.innerHTML = contracts.map(c => {
            const { total, used, balance, pctUsed, isComplete, barColor } = contractStats(c);
            return `<div class="dc-contract-row ${isComplete ? 'dc-contract-done' : ''}">
                <div class="dc-contract-head">
                    <span class="dc-contract-no">${esc(c.contractNo)} · ${esc(c.company)}${isComplete ? ' <span class="cc-done-badge">✓</span>' : ''}</span>
                    <span class="dc-contract-bal" style="color:${isComplete ? '#059669' : balance <= 0 ? '#ff5757' : '#6ee7b7'}">${isComplete ? 'Дууссан' : '₮' + fmtN(balance)}</span>
                </div>
                ...
                <div class="dc-contract-meta">
                    <span>Нийт ₮${fmtN(total)}</span>
                    <span>${isComplete ? '100% дууссан' : pctUsed + '% ашигласан'}</span>
                </div>
            </div>`;
        }).join('');
    }
    ...
    async function applyContractDeduction(a, isPartialAct, isRemainingAct) {
        try {
            ...
            if (total > 0 && newUsed >= total) {
                contractUpd.status = 'completed';
                contractUpd.completedAt = serverTimestamp();
            }
    
            await updateDoc(doc(db, 'contracts', c.id), contractUpd);
            console.log('[ГЭРЭЭ ХАСАЛТ] ✅ Амжилттай. ' + prevUsed + ' → ' + newUsed + ' (₮' + approvedNow + ' нэмэгдсэн)'
                + (contractUpd.status === 'completed' ? ' · Гэрээ 100% дууссан' : ''));
        } catch (err) {
    ...
    async function chooseEbrPaid(docId) {
        const savedId = docId || currentNotaryActId;
        const a = acts.find(x => x.id === savedId);
    ...
    async function chooseEbrUnpaid(docId) {
        const savedId = docId || currentNotaryActId;
        const a = acts.find(x => x.id === savedId);
        if (!a) return;
    ...
    async function approve(docId, ebrAlreadyChecked) {
        ...
        if (role === 'Ерөнхий Нягтлан' && (a.step || 0) === 4 && !a.ebrStatus && !ebrAlreadyChecked) {
            if (a.ebrPdfs && a.ebrPdfs.length) {
                ...
                toast('Е-баримтын төлбөр төлсөн эсэхийг сонгоно уу', 'err');
            }
            return;
        }
        ...
        try {
            ...
            if (finishedAllSteps) {
                ...
                await sendMail(CHAIN[5].email, CHAIN[5].name, a, cu.displayName || cu.email);   // ← Баярмаа руу
                await sendMail(CHAIN[3].email, CHAIN[3].name, a, cu.displayName || cu.email);  // Захирал
    ...
    function openPartialModal(docId, opts = {}) {
        const a = acts.find(x => x.id === docId);
        if (!a) return;
        if (role !== 'Төслийн Координатор') { toast('Зөвхөн Төслийн Координатор хэсэгчлэн батлах эрхтэй', 'err'); return; }
    ...
    function detH(idx) {
        ...
        const canApprove = (a.status === 'pending' || a.status === 'partial')
            ...
            && !isViewer(role)
            && RSTEP[role] === step;
        const canPartial = a.status === 'pending' && role === 'Төслийн Координатор' && step === 0;
        ...
        const needsNotaryCheck = (a.status === 'pending' || a.status === 'partial')
            && role === 'Ерөнхий Нягтлан'
            && step === 4
            && !a.ebrStatus;
    
        const showEbrInlineReadonly = a.ebrPdfs && a.ebrPdfs.length
            && (role === 'Төслийн Координатор' || role === 'Ерөнхий Инженер' || role === 'Ерөнхий Захирал')
            && (a.step || 0) < 4;
        ...
        if (needsNotaryCheck && a.ebrPdfs && a.ebrPdfs.length) {
            ...
            notaryPreviewHtml = `<div class="det-notary-preview det-notary-empty">
                ...
                <div class="det-notary-empty-text">Гүйцэтгэгч баримт оруулаагүй. Та өөрийн мэдээллээр шалгаад доорх сонголтоос төлсөн эсвэл ${EBR_DEDUCT_PERCENT}% хасах (-₮${fmtN(deduction)}) гэж шийднэ үү.</div>
            </div>`;
        ...
        if (needsNotaryCheck) {
            if (a.ebrPdfs && a.ebrPdfs.length) {
                ...
                const deduction = Math.round(contractTotal * EBR_DEDUCT_PERCENT / 100);
                notaryActionHtml = `<div class="abtns" style="border-bottom:1px solid var(--border-subtle)">
                    <button class="bok" onclick="quickEbrPaid('${a.id}')">✓ Е-баримт төлсөн</button>
                    <button class="bpartial-btn" onclick="quickEbrUnpaid('${a.id}')">✗ Төлөөгүй (${EBR_DEDUCT_PERCENT}% хасах)</button>
                    <button class="bno" onclick="reject('${a.id}')">✕ Буцаах</button>
                </div>
                <div class="det-ebr-hint">Дүн хэвээр: ₮${fmtN(a.amount)} · Хасвал: ₮${fmtN(parseInt(a.amount) - deduction)} (-₮${fmtN(deduction)})</div>`;
            }
        }
    ...
    function liH(a, idx, from) {
        ...
        const needsCheck = role === 'Ерөнхий Нягтлан'
    ...
    function renderListBodyHtml() {
        ...
        const stepLabels = {
            coordinator: 'Төслийн Координатор',
